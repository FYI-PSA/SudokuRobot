...
class Solver():
    ...
    def solve_by_candid(self, inputcandids, inputgrid) -> list:
        ...
        for i, row in enumerate(candid):
            for j, item in enumerate(row):
                if not isinstance(item, int):
                    # itemsuperpos = set(map(int, item.split('-')))
    ...
    def simple_solve(self, inputgrid) -> tuple:
        # takes a normal unsolved grid, and tries to solve it using the two functions above. returns a potentially condidate-containing grid and a potentially solved grid.
        grid = deepcopy(inputgrid)
        candid = deepcopy(grid)
    ...
    def OLD_GuessworkSolve(self, gridbase, debug=False) -> tuple:  # pylint: disable=C0103
        candid, grid = self.simple_solve(gridbase)
        ...
        for i, row in enumerate(grid):
            for j, item in enumerate(row):
                ...
                for p in candidates:
                    ...
                    if couldbesolved and self.check_valid_grid(answer):
                        return (True, answer)
    ...
    def count_solve(self, inputgrid) -> tuple:
        candid, grid = self.simple_solve(inputgrid)
        if not self.check_valid_grid(grid):
            return (False, grid)
        for i, row in enumerate(grid):
            for j, item in enumerate(row):
                ...
                for p in candidates:
                    ...
                    testgrid = deepcopy(grid)
                    testgrid[i][j] = p
                    couldbesolved, answer = self.count_solve(testgrid)
                    if couldbesolved and self.check_valid_grid(answer):
                        # return (True, answer)
                        expanded_answer = []
                        for row in answer:
    ...
    def check_valid_grid(self, inputgrid) -> bool:
        ...
        for i, j in self.CHECK_CELLS:
            ...
            box_j = j // 3
            box_neigh = []
            for row_ in inputgrid[box_i*3:(box_i+1)*3]:
                for item_ in row_[box_j*3:(box_j+1)*3]:
                    if item_ == 0:
                        continue
...
def servermain(filename, ai_model, predict_grayscale_func):
    ...
    try:
        count_of_solutions, returnedgrid = main(model=ai_model, filename=filename, predict_grayscale_func=predict_grayscale_func)
        print("got a result")
